...
const ApplicationsSection = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      ...
      console.log(">>> [DEBUG] userData.jobIds:", userData?.jobIds);
      console.log(">>> [DEBUG] applicationsData:", applicationsData);
      // 🧩 Thêm log kiểm tra dữ liệu

      if (isRecruiter) {
        // const recruiterApplications = applicationsData.filter((application) =>
        //   // userData?.jobIds.includes(application.jobId) /// do not work because jobId is ObjectId
        //   userData?.jobIds.map(id => id.toString()).includes(application.jobId.toString())

        // );⭐⭐⭐⭐⭐ 

        const recruiterApplications = applicationsData.filter((application) => {
        const jobIds = userData?.jobIds?.map((id) => id.toString()) || [];
        const match = jobIds.includes(application.jobId.toString());

        console.log(
          `Ứng viên ${application.name} | jobId: ${application.jobId} | match: ${match}`
        );

        return match;
        });

      console.log(">>> [DEBUG] recruiterApplications:", recruiterApplications);
